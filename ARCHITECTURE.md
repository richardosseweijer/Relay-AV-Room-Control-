...
Relay **0.9.63** (beta). Technical overview of the room-control application: process model, data objects, execution path from the operator surface to a device transport, persistence, and the source files that implement each layer.
...
| Widget | `button`, `slider`, `label`, `status`, `schedule`, `preview`, or `image`. Icons sit on the right, sized from tile height. Non-preview/non-image tiles may set `textSize` `sm`|`md`|`lg` (default `md`; font size is a fraction of tile height via `cqh`). Labels may set `hideWhenDisabled` so a failed enable-when omits the tile (default: still show, muted). Image is a static tile (host media under `data/media/`, fit `contain`\|`cover`, optional tap macro). |
