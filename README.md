...
Relay **0.9.65** (beta). Room controller for local AV devices. Private LAN only. Tag `v0.9.65` is a snapshot of this tree (slider textSize removed; fixed face sizing); install and update from **`main`**. After Update, Configurator → Room shows `0.9.65 (<git sha>)`. Versions are three-part from this release.
