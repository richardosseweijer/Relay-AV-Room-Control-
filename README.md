...
Relay **0.9.8** (beta). Room controller for local AV devices. Private LAN only. Tag `v0.9.8` is a snapshot of this tree; install and update from **`main`**. After Update, Configurator → Room shows `0.9.8 (<git sha>)`. Versions are three-part from this release.
