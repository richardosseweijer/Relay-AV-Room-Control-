...
Relay **0.9.56** (beta). Room controller for local AV devices. Private LAN only. Tag `v0.9.56` is a snapshot of this tree (install-host preflight I4); install and update from **`main`**. After Update, Configurator → Room shows `0.9.56 (<git sha>)`. Versions are three-part from this release.
