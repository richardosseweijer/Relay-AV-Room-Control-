...
Relay **0.9.67** (beta). Room controller for local AV devices. Private LAN only. Tag `v0.9.67` is a snapshot of this tree (panel widget labels expand `{var}`); install and update from **`main`**. After Update, Configurator → Room shows `0.9.67 (<git sha>)`. Versions are three-part from this release.
