...
Relay **0.9.69** (beta). Room controller for local AV devices. Private LAN only. Tag `v0.9.69` is a snapshot of this tree (Pages editor shows selected page UI id); install and update from **`main`**. After Update, Configurator → Room shows `0.9.69 (<git sha>)`. Versions are three-part from this release.
