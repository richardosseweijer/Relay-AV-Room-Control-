...
Relay **0.9.53** (beta). Room controller for local AV devices. Private LAN only. Tag `v0.9.53` is a snapshot of this tree (docs/Foyer contract sync after strict peer + device + samsung-pair TLS); install and update from **`main`**. After Update, Configurator → Room shows `0.9.53 (<git sha>)`. Versions are three-part from this release.
