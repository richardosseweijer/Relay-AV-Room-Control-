...
- [ ] **AV-LAN bind + Room-panel URL** — Relay listens on live AV-LAN IPv4 `:8081`; Foyer Setup Relay URL / Room panel URL = `http://<av-lan-ipv4>:8081` (never `0.0.0.0`; not `127.0.0.1` for peer occupancy on dual-NIC). After **Apply AV-LAN IPv4** on Relay (`0.9.59+`), co-hosted Foyer `relayUrl` / `foyer-kiosk.env` are soft-updated when the current value is empty, loopback, or the previous AV IP (deliberate remote URLs are left alone); ufw 8081-from-CIDR is soft-updated the same way (LINUX.md §5b).
