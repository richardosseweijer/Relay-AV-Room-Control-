...
- Empty schedule `days` means every day. See issue #1.
- Nested macros share one `runningMacro` lock; a change-trigger during a run is queued or skipped. See issue #2.
- Driver `pacing.minIntervalMs` is per device send, not a global bus scheduler.
- PINs and tokens are plaintext on disk. See issue #3.
