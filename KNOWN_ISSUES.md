...
- Panel/API listen is the **AV-LAN IPv4** (auto-maps first scanned NIC when AV unset/invalid on fresh install; else loopback only if no NICs). Never `0.0.0.0`. Do not port-forward the panel to venue/WAN. Restrict 8081 to the AV-LAN CIDR on the host firewall (`LINUX.md` §5b). Outbound **None** disables Update. Unsigned `GET /api/peer` is allowed only from TCP loopback, not because `Host` says localhost.
- Panel/API cleartext is AV-LAN only. Optional venue HTTPS (B1 file PEMs) + HMAC peer over that HTTPS (B3) + per-device `nicFace` bind (B4). LE/ACME/DNS-01 **parked**; in-box Generate + Networks UI / CA download / regenerate lifecycle (C1–C3) **shipped** (C4 docs checkpoint `v0.9.46`). **Strict peer TLS** (`v0.9.47`) + **strict device TLS** (`v0.9.48`) + **strict samsung-pair TLS** (`v0.9.49`): venue peers require trusted peer CA; venue device HTTPS/TLS-WS require device CA or sha256 pin (fail-closed); manual `scripts/samsung-pair.mjs` fail-closed on 8002 unless `--ca` / `--fingerprint` (or `--insecure` discover). Soft `rejectUnauthorized: false` is not the venue / pairing happy path. Residual: AV-only Cast soft TLS (blocked on venue). See issue #15 and [`SECURITY.md` Venue TLS inventory](SECURITY.md#venue-tls-inventory-c0).
- ~~Foyer loopback vs AV-only listen~~ **Fixed:** default/control URL prefers live AV-LAN IPv4 (`controlBaseUrlFrom` / Room → Occupancy hint). Soft-fails when no scanned NICs / no IPv4 (unset auto-maps). Same-PC hairpin to the listen host is treated like loopback for unsigned occupancy GET. Lab escape remains `RELAY_LISTEN_HOST=127.0.0.1`. Do not widen listen to `0.0.0.0`.
