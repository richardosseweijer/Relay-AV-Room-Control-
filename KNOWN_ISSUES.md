...
- Panel/API cleartext is AV-LAN only. Optional venue HTTPS (B1 file PEMs) + HMAC peer over that HTTPS (B3) + per-device `nicFace` bind (B4). LE/ACME/DNS-01 **parked**; in-box Generate + Networks UI / CA download / regenerate lifecycle (C1–C3) **shipped** (C4 docs checkpoint `v0.9.46`). Soft TLS verify (`rejectUnauthorized: false`) for file / venue-CA PEMs — optional strict peer verify not implemented. See issue #15 and [`SECURITY.md` Venue TLS inventory](SECURITY.md#venue-tls-inventory-c0).
- AV-LAN listen (A2) does not accept `127.0.0.1`. Foyer’s default Relay URL is still `http://127.0.0.1:8081` and fail-closes non-loopback — occupancy pull breaks until dual-bind / URL story is fixed. **Intentional through B5** (Phase B software checkpoint); not fixed here — do not widen listen to `0.0.0.0`. Lab escape: `RELAY_LISTEN_HOST=127.0.0.1` (not for production dual-NIC).
