...
- Samsung Tizen: pair with **Authenticate** on **8002** (Allow popup), store the token, keep port **8002**. HTTP `:8001/api/v2/` is discovery, not key inject. Power-on from cold needs WOL + MAC. User-installed apps (Netflix etc.) need **Sync inventory** + **App launch**; `KEY_NETFLIX` is the hardware key only.
