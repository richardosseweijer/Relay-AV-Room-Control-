...
- Panel/API cleartext is AV-LAN only. Optional venue HTTPS (B1 file PEMs) + HMAC peer over that HTTPS (B3) + per-device `nicFace` bind (B4). LE/ACME/DNS-01 **parked**; in-box Generate venue CA is **Planned C1–C4** (not shipped). See issue #15 and [`SECURITY.md` Venue TLS inventory](SECURITY.md#venue-tls-inventory-c0).
