...
- ~~Foyer loopback vs AV-only listen~~ **Fixed:** default/control URL prefers live AV-LAN IPv4 (`controlBaseUrlFrom` / Room → Occupancy hint). Soft-fails when AV unset / no IPv4. Same-PC hairpin to the listen host is treated like loopback for unsigned occupancy GET. Lab escape remains `RELAY_LISTEN_HOST=127.0.0.1`. Do not widen listen to `0.0.0.0`.
