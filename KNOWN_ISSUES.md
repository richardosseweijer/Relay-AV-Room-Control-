...
- Panel/API cleartext is AV-LAN only. Optional venue HTTPS (B1 file PEMs) + HMAC peer over that HTTPS (B3) + per-device `nicFace` bind (B4). LE/ACME/DNS-01 **parked**; in-box Generate + Networks UI / CA download / regenerate lifecycle (C1–C3) **shipped** (C4 docs checkpoint `v0.9.46`). **Strict peer TLS** (`v0.9.47`) + **strict device TLS** (`v0.9.48`): venue peers require trusted peer CA; venue device HTTPS/TLS-WS require device CA or sha256 pin (fail-closed). Soft `rejectUnauthorized: false` is not the venue happy path. Residual: AV-only Cast soft TLS (blocked on venue). See issue #15 and [`SECURITY.md` Venue TLS inventory](SECURITY.md#venue-tls-inventory-c0).
