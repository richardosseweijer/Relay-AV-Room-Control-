...
Install **`main`** from GitHub (that is the supported tree). Current package version is **0.8.1** (beta). Confirm with the Room tab version field or `git log -1`. 64-bit Debian, Ubuntu, or Raspberry Pi OS.
...
Do **not** use a zip, an old tag (`v0.7.3`), or a copy of `dist/` from another machine. The in-app update and this guide both track **`origin/main`**. `v0.8.1` is a snapshot of this beta.
