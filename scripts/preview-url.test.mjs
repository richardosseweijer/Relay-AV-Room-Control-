...
test("ffmpeg args stay on flags every static build has", () => {
  ...
  assert.ok(args.includes("-probesize 16384"));
  ...
  const enc = previewFfmpegArgs("rtsp://10.0.25.242:554/sub/av", "udp", undefined, true).join(" ");
  assert.ok(enc.includes("-c:v libx264"));
  assert.ok(enc.includes("-tune zerolatency"));
  assert.ok(enc.includes("-g 10"));
  assert.equal(enc.includes("-c:v copy"), false);
});
