...
test("pages-editor composes PagesImageFields leaf for image widget editor", () => {
  ...
  // Icon picker hidden for image; color swatches hidden when image+borderless
  assert.match(src, /selected\.type !== "image"/);
  assert.match(src, /fills\[color\]/);
  assert.match(src, /selected\.type === "image" && selected\.imageBorderless/);
  ...
  assert.match(leaf, /Borderless/);
  assert.match(leaf, /imageBorderless/);
