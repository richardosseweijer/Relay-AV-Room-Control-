...
test("kiosk disable argv is disable --now fixed unit allowlist", () => {
  const steps = kioskDisableCommands();
  assert.ok(steps.length >= 2);
  for (const step of steps) {
    assert.ok(step.args.includes("disable"));
    assert.ok(step.args.includes("--now"));
    assert.ok(step.args.includes(KIOSK_UNIT));
    assert.equal(step.args.includes("restart"), false);
    assert.equal(
      step.args.some((arg) => /[\s;|&]/.test(arg) || arg.includes("..")),
      false,
    );
  }
  assert.deepEqual(steps[0].args, ["disable", "--now", KIOSK_UNIT]);
  assert.equal(steps[1].args[0], "-n");
  assert.ok(steps[1].args.includes("disable"));
  assert.ok(steps[1].args.includes("--now"));
});
...
test("disableLocalOutput: non-linux does not spawn", () => {
  let called = 0;
  const res = disableLocalOutput({
    platform: "win32",
    spawnSync: () => {
      called += 1;
      return { status: 0 };
...
test("disableLocalOutput: tries systemctl disable --now then sudo -n", () => {
  const calls = [];
  const res = disableLocalOutput({
    platform: "linux",
    spawnSync: (bin, args) => {
  ...
  assert.equal(res.ok, true);
  assert.equal(calls.length, 2);
  assert.deepEqual(calls[0].args, ["disable", "--now", KIOSK_UNIT]);
  assert.equal(calls[1].args[0], "-n");
  assert.ok(calls[1].args.includes("disable"));
  assert.ok(calls[1].args.includes("--now"));
  assert.ok(calls[1].args.includes(KIOSK_UNIT));
});
...
test("enableLocalOutput: polkit / missing sudoers → clear LINUX.md hint", () => {
  ...
  assert.match(res.detail, /install-host-sudoers\.sh/);
});

test("disableLocalOutput: polkit / missing sudoers → clear LINUX.md / install-host-sudoers hint", () => {
  const polkit =
    "Failed to disable unit: Access denied as the requested operation requires interactive authentication.";
  const sudoPw = "sudo: a password is required";
  const res = disableLocalOutput({
    platform: "linux",
    spawnSync: (_bin, args) => {
  ...
  assert.equal(res.reason, "sudoers");
  assert.equal(res.detail, KIOSK_SUDOERS);
  assert.match(res.detail, /LINUX\.md §7/);
  assert.match(res.detail, /install-host-sudoers\.sh/);
});
...
test("sudoers.relay-kiosk is narrow systemctl allowlist including disable --now", () => {
  ...
  assert.match(body, /\/usr\/bin\/systemctl disable --now relay-kiosk\.service/);
  assert.match(body, /\/usr\/bin\/systemctl enable --now relay-kiosk\.service/);
});
