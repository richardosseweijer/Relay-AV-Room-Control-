...
## Unreleased
- **Venue TLS C3 (lifecycle):** Networks **Regenerate** requires explicit confirm before replacing CA/leaf; re-issues for current live NIC2 IPv4 and reloads venue HTTPS only (AV untouched). Expiry UX shows days-left and warns at ≤30 days / expired with a clear Regenerate path. Strengthened IP-drift mismatch → confirm → Regenerate. Light auto-check on Networks load + Refresh NICs recomputes mismatch/expiry flags — **no silent auto-reissue**. C4 remains Planned (full doc audit + checkpoint tag).
- **Venue TLS C2 (Networks UI):** Room → Networks **Generate venue certificate** (disabled + reason when outbound None / no live IPv4); status from `getVenueTlsStatus` (active / SAN IP / expiry / fingerprint); same-origin **Download CA** via `/api/venue-tls-ca` (config-token; `ca.cert.pem` only); IP-mismatch banner when live NIC2 IPv4 ∉ leaf SAN; brief iOS/Android/Windows/macOS CA install hints. C3 lifecycle shipped (see Unreleased); C4 remains Planned (doc audit + tag).
