...
## Unreleased
- **Venue TLS C2 (Networks UI):** Room → Networks **Generate venue certificate** (disabled + reason when outbound None / no live IPv4); status from `getVenueTlsStatus` (active / SAN IP / expiry / fingerprint); same-origin **Download CA** via `/api/venue-tls-ca` (config-token; `ca.cert.pem` only); IP-mismatch banner when live NIC2 IPv4 ∉ leaf SAN; brief iOS/Android/Windows/macOS CA install hints. C3–C4 remain Planned (regenerate confirm / expiry / doc tag).
