...
- Optional Preview widget: host `ffmpeg` remuxes RTSP (ZowieBox `rtsp://IP:8554/sub/av`, 720p H.264, `-c:v copy`) to fMP4 for a panel `<video>` tile. No extra npm. Missing ffmpeg shows `ffmpeg?`. Cap four streams. Drop the files listed at the top of `src/lib/control/preview-grab.ts` to remove it. ffmpeg 7 takes `-timeout`, not `-rw_timeout`.
