...
export const extraDrivers: Record<string, DriverSpec> = {
  ...
  "extron-mps-602.json": {
    ...
    device: {
      ...
      model: "MPS 602",
      type: "switcher",
      notes: "No Ethernet on the switcher. SIS is RS-232 9600 8N1 on the rear captive-screw port (or USB Config). Behind an IPL, bind this device to the IPL COM slot (TCP 2001/2002). Simple commands (! V Z B) are literal. Tagged commands: manual E is ESC (0x1B), } is CR. Inputs: 1-2 VGA, 3-5 HDMI, 6 DTP. Volume 0-100. 1Z mutes program audio (not mic). Signal presence: ESC 0LS CR, reply Sig in1..in6 * rgb hdmi.",
    },
    transports: {
      lan: { protocol: "tcp", port: 2001, timeoutMs: 8000, session: { keepMs: 15000 } },
    ...
    commands: [
      ...
      { id: "auto.off", label: "Auto-switch off", kind: "action", transport: "lan", payload: "\x1b0AUSW\r" },
      { id: "auto.high", label: "Auto-switch highest", kind: "action", transport: "lan", payload: "\x1b1AUSW\r" },
      { id: "auto.low", label: "Auto-switch lowest", kind: "action", transport: "lan", payload: "\x1b2AUSW\r" },
    ...
    feedback: [
      ...
      { id: "signal.1", label: "VGA 1 signal", kind: "enum", values: ["off", "on"], transport: "lan", mode: "poll", query: "\x1b0LS\r", pollMs: 4000, parse: { type: "regex", pattern: "Sig\\s*([01])", map: { "0": "off", "1": "on" } } },
      { id: "signal.2", label: "VGA 2 signal", kind: "enum", values: ["off", "on"], transport: "lan", mode: "poll", query: "\x1b0LS\r", pollMs: 4000, parse: { type: "regex", pattern: "Sig\\s*[01][\\s*]*([01])", map: { "0": "off", "1": "on" } } },
      { id: "signal.3", label: "HDMI 3 signal", kind: "enum", values: ["off", "on"], transport: "lan", mode: "poll", query: "\x1b0LS\r", pollMs: 4000, parse: { type: "regex", pattern: "Sig\\s*(?:[01][\\s*]*){2}([01])", map: { "0": "off", "1": "on" } } },
      { id: "signal.4", label: "HDMI 4 signal", kind: "enum", values: ["off", "on"], transport: "lan", mode: "poll", query: "\x1b0LS\r", pollMs: 4000, parse: { type: "regex", pattern: "Sig\\s*(?:[01][\\s*]*){3}([01])", map: { "0": "off", "1": "on" } } },
      { id: "signal.5", label: "HDMI 5 signal", kind: "enum", values: ["off", "on"], transport: "lan", mode: "poll", query: "\x1b0LS\r", pollMs: 4000, parse: { type: "regex", pattern: "Sig\\s*(?:[01][\\s*]*){4}([01])", map: { "0": "off", "1": "on" } } },
      { id: "signal.6", label: "DTP 6 signal", kind: "enum", values: ["off", "on"], transport: "lan", mode: "poll", query: "\x1b0LS\r", pollMs: 4000, parse: { type: "regex", pattern: "Sig\\s*(?:[01][\\s*]*){5}([01])", map: { "0": "off", "1": "on" } } },
    ],
  },
  "wake-on-lan.json": {
    specVersion: "2",
