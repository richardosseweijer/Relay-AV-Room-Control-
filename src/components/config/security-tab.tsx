  ...
  return (
    <section className="grid gap-4 sm:grid-cols-2">
            <label className="grid gap-1 text-sm text-muted">Config PIN
              <input className={fieldClass()} type="password" inputMode="numeric" pattern="[0-9]*" autoComplete="off" placeholder="unchanged" value={String(draft.room.configPin || "").startsWith("scrypt$") ? "" : draft.room.configPin} onChange={(e) => update((c) => { c.room.configPin = e.target.value; })} />
            </label>
            ...
            {draft.room.panelAccess === "pin" ? (
              <label className="grid gap-1 text-sm text-muted">Panel PIN
                <input className={fieldClass()} type="password" inputMode="numeric" pattern="[0-9]*" autoComplete="off" placeholder="unchanged" value={String(draft.room.panelPin ?? "").startsWith("scrypt$") ? "" : (draft.room.panelPin ?? "")} onChange={(e) => update((c) => { c.room.panelPin = e.target.value; })} />
              </label>
            ) : null}
