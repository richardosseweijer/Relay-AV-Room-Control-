  ...
  if (mustChange) {
    return (
      <main className="mx-auto flex min-h-dvh max-w-sm flex-col justify-center gap-4 bg-bg px-6">
        ...
        <input className={fieldClass()} type="tel" inputMode="numeric" pattern="[0-9]*" autoComplete="new-password" value={newPin} onChange={(e) => setNewPin(e.target.value)} placeholder="New PIN" />
        <input className={fieldClass()} type="tel" inputMode="numeric" pattern="[0-9]*" autoComplete="new-password" value={newPin2} onChange={(e) => setNewPin2(e.target.value)} placeholder="Repeat PIN" />
        ...
        <div className="fixed right-4 top-4 z-50 max-w-sm rounded-xl border border-border bg-surface p-4">
          ...
          {needLock ? (
            <div className="mt-3 grid gap-2">
              <input className={fieldClass()} type="tel" inputMode="numeric" pattern="[0-9]*" autoComplete="one-time-code" value={lockPin} onChange={(e) => setLockPin(e.target.value)} placeholder="PIN" />
              <Button onClick={async () => {
      ...
      {gate ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-bg/80 px-4">
          <div className="w-full max-w-sm rounded-xl border border-border bg-surface p-5">
            ...
            <input className={`${fieldClass()} mt-4`} type="tel" inputMode="numeric" pattern="[0-9]*" autoComplete="one-time-code" value={gate.pin} onChange={(e) => setGate({ ...gate, pin: e.target.value })} placeholder="PIN" />
