...
Relay **0.9.71** (beta). Single-process LAN AV room controller. Phase B software checkpoint (B5 at `v0.9.45`). LE/ACME parked; C1–C4 in-box venue TLS train closed (Generate + Networks UI + lifecycle + docs checkpoint).
...
| Panel label `{var}` / `\n` | `vars.ts` (`resolveTemplate` / `resolveWidgetLabel` / `formatWidgetLabel`), `panel-tile.tsx` + face shells (`whitespace-pre-line`; unresolved → empty) |
...
## Live notes (still true at 0.9.71)
