...
Relay **0.9.63** (beta). Single-process LAN AV room controller. Phase B software checkpoint (B5 at `v0.9.45`). LE/ACME parked; C1–C4 in-box venue TLS train closed (Generate + Networks UI + lifecycle + docs checkpoint).
...
| Panel text size | `text-size-widget.ts`, `pages-editor.tsx`, `widget-face.tsx` / `panel-tile.tsx` / `panel-slider.tsx` |
...
## Live notes (still true at 0.9.63)
