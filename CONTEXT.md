...
Relay **0.9.46** (beta). Single-process LAN AV room controller. Phase B software checkpoint (B5 at `v0.9.45`). LE/ACME parked; C1–C4 in-box venue TLS train closed (Generate + Networks UI + lifecycle + docs checkpoint).
...
## Live foot-guns (still true at 0.9.46)
...
| 15 | HTTP on AV-LAN; venue HTTPS B1 + peer B3 + nicFace B4 (B5); LE parked; Generate C1–C3 + C4 docs checkpoint shipped (`v0.9.46`) |
