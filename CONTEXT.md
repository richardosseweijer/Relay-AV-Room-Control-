...
Relay **0.9.47** (beta). Single-process LAN AV room controller. Phase B software checkpoint (B5 at `v0.9.45`). LE/ACME parked; C1–C4 in-box venue TLS train closed (Generate + Networks UI + lifecycle + docs checkpoint).
...
## Live notes (still true at 0.9.47)
...
| 15 | HTTP on AV-LAN; venue HTTPS B1 + peer B3 (strict CA) + nicFace B4 (B5); LE parked; Generate C1–C4 shipped; strict peer TLS `v0.9.47` |
