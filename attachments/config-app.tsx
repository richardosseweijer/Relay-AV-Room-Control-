...
export function ConfigApp() {
  ...
  function flash(title: string, body: string) {
    setToast({ title, body });
    if (/lock required|config lock/i.test(`${title} ${body}`)) {
      setNeedLock(true);
      setLockPin("");
    }
  }
          ...
          <section className="grid gap-4 sm:grid-cols-2">
            ...
            <div className="sm:col-span-2 flex flex-wrap gap-2">
              ...
              <Button variant="secondary" onClick={async () => {
                ...
                const res = await restartHost({ data: { token } });
              ...
              <Button variant="danger" onClick={async () => {
                ...
                const res = await rebootHost({ data: { token } });
                ...
                <article key={device.id} className="rounded-xl border border-border bg-surface p-4">
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    ...
                    {driver?.inventory?.resources?.length ? (
                      <Button size="sm" variant="secondary" onClick={async () => {
                        const res = await pullInventory({ data: { token, deviceId: device.id, host: device.host, port: device.port, driver: device.driver, auth: device.auth, simulate: device.simulate } });
                    ...
                    <Button size="sm" variant="secondary" onClick={async () => {
                      ...
                      const res = await debugScan({ data: { token, host: device.host, ports } });
                  ...
                  <Button size="sm" variant="secondary" className="mt-2" onClick={async () => {
                    ...
                    const res = await debugSend({ data: { token, deviceId: device.id, payload: el?.value ?? "", host: device.host, port: device.port, driver: device.driver, auth: device.auth } });
          ...
          <section className="grid gap-3">
            <div className="flex flex-wrap items-end gap-2">
              ...
              <Button variant="secondary" onClick={async () => {
                ...
                const res = await addDriverFromLibrary({ data: { token, filename: libraryPick } });
                  ...
                  <div className="mt-2 grid gap-2">
                    ...
                    <div className="flex gap-2">
                      <Button size="sm" onClick={async () => {
                        ...
                        await saveDriver({ data: { token, filename: name, spec: parsed } });
            ...
            <label className="inline-flex h-11 w-fit items-center rounded-md border border-border bg-surface px-4 text-sm">
              ...
              <input type="file" accept="application/json" className="hidden" onChange={async (e) => {
                ...
                await saveDriver({ data: { token, filename: file.name, spec } });
    ...
    <section className="grid gap-4 lg:grid-cols-[1fr_20rem]">
        ...
        <div className="relative grid gap-1" style={{ gridTemplateColumns: `repeat(${page.grid.cols}, minmax(0, 1fr))`, gridTemplateRows: `repeat(${page.grid.rows}, 3.2rem)` }}>
          ...
          {page.widgets.map((widget) => (
            <button
              ...
              className={cn("z-10 cursor-grab rounded-md border px-2 text-left text-xs active:cursor-grabbing", selectedId === widget.id ? "border-accent" : "border-border", fills[widget.color])}
      ...
      {selected ? (
        <aside className="grid gap-2 rounded-xl border border-border bg-surface p-4">
          ...
          <label className="text-sm text-muted">Enable when variable
            <select className={fieldClass()} value={selected.enableWhen?.variable ?? ""} onChange={(e) => update((c) => {
              const w = c.pages.find((p) => p.id === page.id)?.widgets.find((item) => item.id === selected.id);
              if (!w) return;
              w.enableWhen = e.target.value ? { variable: e.target.value, equals: w.enableWhen?.equals ?? "", device: undefined, feedback: undefined } : null;
            })}>
          ...
          {selected.enableWhen?.variable ? (
            <label className="grid gap-1 text-sm text-muted">Equals
            <input className={fieldClass()} value={selected.enableWhen.equals} onChange={(e) => update((c) => { const w = c.pages.find((p) => p.id === page.id)?.widgets.find((item) => item.id === selected.id); if (w?.enableWhen) w.enableWhen.equals = e.target.value; })} />
            </label>
          ) : null}
